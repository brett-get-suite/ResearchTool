...
import {
  ...
  mockFetchHourlyPerformance,
  mockFetchAssetPerformance,
...
export async function fetchHourlyPerformance(client, dateRange = 'LAST_30_DAYS') {
  if (MOCK_MODE) return mockFetchHourlyPerformance();
...
/**
 * Fetch asset performance labels from ad_group_ad_asset_view.
 * Covers RSAs, Demand Gen, and App Ads. NOT Performance Max
 * (PMax uses asset_group_asset which is a separate resource).
 *
 * Returns flat array of assets with performance label, type, field type,
 * campaign context, and preview data (image URL / video ID / text).
 */
export async function fetchAssetPerformance(client) {
  if (MOCK_MODE) return mockFetchAssetPerformance();
  ...
  return (rows || []).map((row) => ({
    ...
    videoId: row.asset?.youtubeVideoAsset?.youtubeVideoId || null,
