...
async function mutate(client, operations) {
  if (MOCK_MODE) return mockMutateResponse();
...
export async function createCampaign(client, { name, budgetResourceName, biddingStrategy, networkSettings, status = 'PAUSED', targetCpaMicros = 5_000_000 }) {
  const campaign = {
    name,
    status,
  ...
  if (biddingStrategy === 'MAXIMIZE_CONVERSIONS') {
    ...
    campaign.targetCpa = { targetCpaMicros: String(targetCpaMicros) };
  } else {
