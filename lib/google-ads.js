 ...
 *
 * Set GOOGLE_ADS_MOCK_MODE=true in Vercel env vars to return realistic
 * demo data without a real Google Ads account.
 */

import {
  MOCK_MODE,
  mockResolveGeoTargets,
  mockGetKeywordMetrics,
...
export async function resolveGeoTargets(serviceAreas) {
  if (MOCK_MODE) return mockResolveGeoTargets();
...
export async function getKeywordMetrics(keywords, geoTargets = [US_NATIONAL]) {
  if (MOCK_MODE) return mockGetKeywordMetrics(keywords);
  ...
  for (let i = 0; i < keywords.length; i += BATCH) {
    ...
    console.log('Keyword Planner request geoTargets:', JSON.stringify(geoTargets));
    ...
    if (!res.ok) {
      const err = await res.json().catch(() => ({}));
      const details = err.error?.details?.map(d => JSON.stringify(d)).join('; ') || '';
      const msg = err.error?.message || err.error?.status || res.statusText;
      console.error('Keyword Planner full error:', JSON.stringify(err, null, 2));
      throw new Error(`Keyword Planner API error (${res.status}): ${msg}${details ? ` | ${details}` : ''}`);
    }
...
export async function generateKeywordIdeas(seedKeywords, geoTargets = [US_NATIONAL], pageSize = 100) {
  if (MOCK_MODE) return mockGenerateKeywordIdeas(seedKeywords);
