...
export function budgetProjectionPrompt(businessName, industry, serviceAreas, keywordData, competitorData, calibration = null, seasonalMultipliers = null) {
  const calibrationBlock = calibration?.spend > 0 && calibration?.leads > 0
...
${calibrationBlock}${seasonalBlock}
...
URL: <data>${url}</data>
CONTENT SAMPLE: <data>${websiteContent?.slice(0, 2000) || '(not available)'}</data>
...
CAMPAIGNS: <data>${JSON.stringify(campaigns?.slice(0, 20))}</data>
AD GROUPS: <data>${JSON.stringify(adGroups?.slice(0, 30))}</data>
KEYWORDS (sample): <data>${JSON.stringify(keywords?.slice(0, 50))}</data>
ADS (sample): <data>${JSON.stringify(ads?.slice(0, 20))}</data>
CAMPAIGN METRICS: <data>${JSON.stringify(metrics)}</data>
...
KEYWORDS WITH METRICS: <data>${JSON.stringify(keywords?.slice(0, 100))}</data>
SEARCH TERMS REPORT: <data>${JSON.stringify(searchTerms?.slice(0, 100))}</data>
BRAND PROFILE: <data>${JSON.stringify(brandProfile)}</data>
...
KEYWORDS WITH METRICS: <data>${JSON.stringify(keywords?.slice(0, 100))}</data>
GOALS: <data>${JSON.stringify(goals || { target_cpa: null, maximize: 'conversions' })}</data>
...
CAMPAIGNS WITH METRICS: <data>${JSON.stringify(metrics)}</data>
CURRENT BUDGETS: <data>${JSON.stringify(campaigns?.map(c => ({ name: c.name, budgetAmountMicros: c.budgetAmountMicros, budgetResource: c.budgetResource })))}</data>
...
AD GROUP: <data>${adGroup.name}</data>
KEYWORDS: <data>${JSON.stringify(keywords?.slice(0, 10)?.map(k => k.keyword))}</data>
BRAND PROFILE: <data>${JSON.stringify(brandProfile)}</data>
EXISTING ADS: <data>${JSON.stringify(existingAds?.slice(0, 3))}</data>
...
SEARCH TERMS (with spend): <data>${JSON.stringify(searchTerms?.slice(0, 100))}</data>
EXISTING NEGATIVES: <data>${JSON.stringify(existingNegatives?.slice(0, 50))}</data>
BUSINESS CONTEXT: <data>${JSON.stringify(brandProfile)}</data>
