...
export function mockFetchCampaignMetricsWithIS() {
  return [
    {
      campaignId: '11111111', name: 'HVAC Services - Search', status: 'ENABLED', type: 'SEARCH',
      budget: 5000, impressions: 18420, clicks: 1103, spend: 21847, conversions: 84, ctr: 0.0599, avgCpc: 19.81,
      cpa: 260.08, search_impression_share: 0.72, search_top_impression_share: 0.58, search_abs_top_impression_share: 0.31,
    },
    {
      campaignId: '22222222', name: 'AC Repair - Brand', status: 'ENABLED', type: 'SEARCH',
      budget: 2000, impressions: 6840, clicks: 892, spend: 9640, conversions: 61, ctr: 0.1304, avgCpc: 10.81,
      cpa: 158.03, search_impression_share: 0.91, search_top_impression_share: 0.84, search_abs_top_impression_share: 0.67,
    },
    {
      campaignId: '33333333', name: 'Plumbing Services - Search', status: 'PAUSED', type: 'SEARCH',
      budget: 3000, impressions: 0, clicks: 0, spend: 0, conversions: 0, ctr: 0, avgCpc: 0,
      cpa: null, search_impression_share: null, search_top_impression_share: null, search_abs_top_impression_share: null,
    },
  ];
}
...
export function mockFetchChangeHistory() {
  const now = Date.now();
  return [
    {
      changeDateTime: new Date(now - 2 * 3600000).toISOString(),
      changeResourceType: 'CAMPAIGN',
      changeResourceName: 'customers/1234567890/campaigns/11111111',
      resourceChangeOperation: 'UPDATE',
      changedFields: 'campaign_budget.amount_micros',
      userEmail: 'manager@agency.com',
      campaign: { id: '11111111', name: 'HVAC Services - Search' },
      oldResource: { campaignBudget: { amountMicros: '4500000000' } },
      newResource: { campaignBudget: { amountMicros: '5000000000' } },
    },
    {
      changeDateTime: new Date(now - 8 * 3600000).toISOString(),
      changeResourceType: 'AD_GROUP_CRITERION',
      changeResourceName: 'customers/1234567890/adGroupCriteria/101~9999',
      resourceChangeOperation: 'CREATE',
      changedFields: 'keyword.text,keyword.match_type',
      userEmail: 'manager@agency.com',
      campaign: { id: '11111111', name: 'HVAC Services - Search' },
      oldResource: null,
      newResource: { keyword: { text: 'emergency ac repair', matchType: 'PHRASE' } },
    },
    {
      changeDateTime: new Date(now - 26 * 3600000).toISOString(),
      changeResourceType: 'AD_GROUP_BID_MODIFIER',
      changeResourceName: 'customers/1234567890/adGroupBidModifiers/101~mobile',
      resourceChangeOperation: 'UPDATE',
      changedFields: 'bid_modifier',
      userEmail: 'external@client.com',
      campaign: { id: '11111111', name: 'HVAC Services - Search' },
      oldResource: { bidModifier: 1.0 },
      newResource: { bidModifier: 1.15 },
    },
  ];
}
...
export function mockFetchHourlyPerformance() {
  const DAYS = [1, 2, 3, 4, 5, 6, 0]; // Mon-Sun
  const rows = [];
  for (const day of DAYS) {
    for (let hour = 0; hour < 24; hour++) {
      const isBusinessHour = hour >= 8 && hour <= 17 && day >= 1 && day <= 5;
      const multiplier = isBusinessHour ? 2.5 : 0.5;
      rows.push({
        day_of_week: day,
        hour_of_day: hour,
        impressions: Math.round(120 * multiplier + Math.random() * 40),
        clicks: Math.round(8 * multiplier + Math.random() * 3),
        conversions: Math.round(1.2 * multiplier + Math.random() * 0.5),
        cost: +(4.5 * multiplier + Math.random() * 2).toFixed(2),
      });
    }
  }
  return rows;
...
// ─── Asset Performance Mock ──────────────────────────────────────

const MOCK_ASSETS = [
  { id: '90001', type: 'IMAGE', fieldType: 'MARKETING_IMAGE', performanceLabel: 'BEST', text: null, imageUrl: 'https://placehold.co/600x314/1a3d2e/4edea3?text=Hero+Image', videoId: null, campaignName: 'Brand - Plumbing Services', adGroupName: 'Emergency Plumbing', campaignType: 'SEARCH' },
  { id: '90002', type: 'IMAGE', fieldType: 'SQUARE_MARKETING_IMAGE', performanceLabel: 'BEST', text: null, imageUrl: 'https://placehold.co/300x300/1a3d2e/4edea3?text=Square+Ad', videoId: null, campaignName: 'Brand - Plumbing Services', adGroupName: 'Emergency Plumbing', campaignType: 'SEARCH' },
  { id: '90003', type: 'YOUTUBE_VIDEO', fieldType: 'VIDEO', performanceLabel: 'GOOD', text: null, imageUrl: null, videoId: 'dQw4w9WgXcQ', campaignName: 'Video - Service Demo', adGroupName: 'Service Videos', campaignType: 'VIDEO' },
  { id: '90004', type: 'TEXT', fieldType: 'HEADLINE', performanceLabel: 'BEST', text: 'Emergency Plumber Near You — 24/7', imageUrl: null, videoId: null, campaignName: 'Brand - Plumbing Services', adGroupName: 'Emergency Plumbing', campaignType: 'SEARCH' },
  { id: '90005', type: 'TEXT', fieldType: 'HEADLINE', performanceLabel: 'GOOD', text: 'Licensed & Insured Plumbers', imageUrl: null, videoId: null, campaignName: 'Brand - Plumbing Services', adGroupName: 'General Plumbing', campaignType: 'SEARCH' },
  { id: '90006', type: 'TEXT', fieldType: 'DESCRIPTION', performanceLabel: 'BEST', text: 'Fast, affordable plumbing repairs. Same-day service available. Call now for a free estimate!', imageUrl: null, videoId: null, campaignName: 'Brand - Plumbing Services', adGroupName: 'Emergency Plumbing', campaignType: 'SEARCH' },
  { id: '90007', type: 'TEXT', fieldType: 'DESCRIPTION', performanceLabel: 'LOW', text: 'We do plumbing. Contact us today.', imageUrl: null, videoId: null, campaignName: 'Brand - Plumbing Services', adGroupName: 'General Plumbing', campaignType: 'SEARCH' },
  { id: '90008', type: 'IMAGE', fieldType: 'LOGO', performanceLabel: 'GOOD', text: null, imageUrl: 'https://placehold.co/128x128/2a2d4a/bdc2ff?text=Logo', videoId: null, campaignName: 'Brand - Plumbing Services', adGroupName: 'Emergency Plumbing', campaignType: 'SEARCH' },
  { id: '90009', type: 'TEXT', fieldType: 'HEADLINE', performanceLabel: 'LEARNING', text: 'Top-Rated Local Plumber', imageUrl: null, videoId: null, campaignName: 'Brand - HVAC Services', adGroupName: 'AC Repair', campaignType: 'SEARCH' },
  { id: '90010', type: 'TEXT', fieldType: 'HEADLINE', performanceLabel: 'GOOD', text: 'Same-Day Water Heater Repair', imageUrl: null, videoId: null, campaignName: 'Brand - Plumbing Services', adGroupName: 'Water Heater', campaignType: 'SEARCH' },
  { id: '90011', type: 'IMAGE', fieldType: 'MARKETING_IMAGE', performanceLabel: 'LOW', text: null, imageUrl: 'https://placehold.co/600x314/3d1f1a/f87171?text=Old+Banner', videoId: null, campaignName: 'Brand - HVAC Services', adGroupName: 'AC Repair', campaignType: 'SEARCH' },
  { id: '90012', type: 'TEXT', fieldType: 'DESCRIPTION', performanceLabel: 'GOOD', text: 'Trusted by 10,000+ homeowners. 5-star rated. Schedule your repair online in 60 seconds.', imageUrl: null, videoId: null, campaignName: 'Brand - HVAC Services', adGroupName: 'Furnace Repair', campaignType: 'SEARCH' },
];

export function mockFetchAssetPerformance() {
  return MOCK_ASSETS;
}

// ─── Write no-ops ─────────────────────────────────────────────────

export function mockMutateResponse(operationCount = 1) {
  return {
    mutateOperationResponses: Array.from({ length: operationCount }, (_, i) => {
      const id = Date.now() + i;
      return {
