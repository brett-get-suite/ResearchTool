...
export async function callGemini(apiKey, prompt, { temperature = 0.3, maxTokens = 32768 } = {}) {
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      if (finishReason === 'MAX_TOKENS') {
        console.warn(`Gemini response truncated (MAX_TOKENS) on attempt ${attempt + 1}`);
      }
      ...
      if (attempt < MAX_RETRIES) {
        ...
        if (err.message?.includes('API key') || err.status === 401 || err.status === 403 || err.status === 400) {
          throw err;
        }
