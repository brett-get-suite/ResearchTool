...
function DeltaArrow({ current, previous, invert = false }) {
  ...
  return (
    <span className={`ds-delta ${isPos ? 'ds-delta--positive' : isNeg ? 'ds-delta--negative' : 'ds-delta--neutral'}`}>
...
/* ── Health badge ── */
const HEALTH = {
  healthy:  { label: 'Healthy',  badge: 'ds-status-badge--success', dot: 'bg-current' },
  warning:  { label: 'Warning',  badge: 'ds-status-badge--warning', dot: 'bg-current' },
  critical: { label: 'Critical', badge: 'ds-status-badge--error',   dot: 'bg-current' },
};

function HealthBadge({ status }) {
  const cfg = HEALTH[status] || HEALTH.healthy;
  return (
    <span className={`ds-status-badge ${cfg.badge}`}>
  ...
  return (
    <div className="bg-surface-card rounded-2xl p-6">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="material-symbols-outlined text-on-surface-variant text-xl opacity-25">{icon}</span>
      </div>
      <div className="ds-secondary-metric">{fmt(value)}</div>
      {prevValue != null && (
...
function AccountCard({ account, metrics: m, campaigns, alertCount, syncing, onSync, onAudit, onPauseAgents }) {
  ...
  return (
    <div className="bg-surface-card rounded-2xl p-6 flex flex-col gap-4 hover:bg-surface-elevated transition-colors">
      {/* Header */}
      ...
      <div className="grid grid-cols-3 gap-2 mt-auto pt-1">
        <Link
          ...
          className="flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-xs font-semibold text-on-primary gradient-primary"
        >
        ...
        <button
          ...
          disabled={syncing}
          className="ds-btn-secondary !text-xs !py-2.5 !px-2 disabled:opacity-40"
        ...
        <button
          ...
          className="ds-btn-secondary !text-xs !py-2.5 !px-2"
          title="Run audit"
        >
  ...
  return (
    <div className="space-y-8 fade-up">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <h2 className="ds-page-title mb-1">Managed Accounts</h2>
      ...
      {!loading && accounts.length > 0 && (
        <div className="flex flex-wrap items-center gap-4 bg-surface-card rounded-2xl px-6 py-4">
          ...
          <div className="flex items-center gap-2">
            {[
              { key: 'all', label: 'All' },
              { key: 'critical', label: `Critical (${healthCounts.critical})`, dot: 'bg-ds-error' },
              { key: 'warning', label: `Warning (${healthCounts.warning})`, dot: 'bg-ds-warning' },
              { key: 'healthy', label: `Healthy (${healthCounts.healthy})`, dot: 'bg-ds-success' },
            ].map(f => (
              <button
                key={f.key}
                onClick={() => setHealthFilter(f.key)}
                className={`ds-filter-chip ${healthFilter === f.key ? 'ds-filter-chip--active' : ''}`}
              >
                {f.dot && <span className={`w-1.5 h-1.5 rounded-full ${healthFilter === f.key ? 'bg-white' : f.dot}`} />}
                {f.label}
              </button>
            ))}
          </div>
          ...
          <select
            ...
            className="text-sm py-2 px-3 rounded-xl bg-surface-container-high border border-outline-variant/30 focus:border-primary/40 outline-none"
          >
      ...
      {loading ? (
        ...
        /* Empty state */
        <div className="bg-surface-card rounded-2xl">
          <div className="ds-empty-state !py-16">
            <span className="material-symbols-outlined ds-empty-state__icon">account_balance</span>
            <p className="ds-empty-state__title !text-xl">No accounts connected yet</p>
            <p className="ds-empty-state__desc !max-w-sm">
              Connect a Google Ads account to start managing campaigns, tracking performance, and unlocking AI-powered insights.
            </p>
            <div className="ds-empty-state__cta">
              <GradientButton onClick={() => setConnectModalOpen(true)}>
                <span className="material-symbols-outlined text-lg">add_link</span>
        ...
        <div className="bg-surface-card rounded-2xl">
          <div className="ds-empty-state">
            <span className="material-symbols-outlined ds-empty-state__icon">filter_list_off</span>
            <p className="ds-empty-state__title">No accounts match your filters</p>
          </div>
        </div>
      ) : viewMode === 'grid' ? (
        /* Grid view */
